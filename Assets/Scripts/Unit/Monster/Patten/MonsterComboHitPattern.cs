...
{
    public class MonsterComboHitPattern : MonsterPattern, IGroggyTrackable
    {
        [Header("Slash Settings")]
        [SerializeField] private bool _fixedThisPos = false;
        ...
        public override void Init(MonsterPhaseController phaseController)
        {
            ...
            {
                _finishHitSpriter = _finishHitScanner.GetComponentInChildren<HitBoxSpriter>();
        ...
        public override bool Condition()
        {
            ...
            return (_attackRange >= distance) &&
                   _normalHitScanner != null &&
        ...
        protected override IEnumerator IE_PlayAction()
        {
            ...
            // 필요 값들
            Vector3 hitBoxPoint = CalculateHitBoxPosition(_thisTr, _targetTr);

            float elapsed = 0f;
            AnimationClip clip = null;

            if (patternCastClips != null)
            {
                clip = patternCastClips;
                phaseController.OwnerController.Anim.PlayClip(clip);
            }
            else
                clip = (0 < _slashClips.Count) ? _slashClips[0] : patternActionClips;
            ...
            while (elapsed < castDelay)
            {
                ...
                if (!_fixedThisPos)
                {
                    hitBoxPoint = CalculateHitBoxPosition(_thisTr, _targetTr);
        ...
        private bool Hit(Collider2D[] buffer)
        {
