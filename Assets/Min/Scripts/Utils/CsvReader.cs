...
using CsvClass = ProjectVS.Utils.Csv.Csv;
using CsvTableClass = ProjectVS.Utils.CsvTable.CsvTable;
using CsvDictionaryClass = ProjectVS.Utils.CsvDictionary.CsvDictionary;


namespace ProjectVS.Utils.CsvReader
{
    /// <summary>
    /// # Summary
    ...
    public static class CsvReader
    {
        ...
        public static void Read(CsvClass csv)
        {
            if (!IsValidPath(csv) ||
            ...
            switch (csv)
            {
                case CsvTableClass table:
                    isReadSuccessful = ReadToTable(table, lines);
                    break;
                case CsvDictionaryClass dictionary:
        ...
        private static bool ReadToDictionary(CsvDictionaryClass csv, string[] lines)
        {
        ...
        /// <param name="lines">The lines read from the CSV file. CSV 파일에서 읽은 줄.</param>
        /// <returns>True if reading was successful, otherwise false. 읽기가 성공하면 true, 그렇지 않으면 false.</returns>
        private static bool ReadToTable(CsvTableClass csv, string[] lines)
        {
        ...
        private static bool IsValidEmpty(CsvClass csv, out string[] lines)
        {
        ...
        private static void PrintResult(CsvClass csv, bool result)
        {
#if UNITY_EDITOR
            if (result)
            {
